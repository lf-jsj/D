...
internal class GachaEventPipeline : IPipeline
{
    ...
    private readonly List<GachaEvent> gachaEvents = new()
    {
        ...
        //3.2上半
        new("3.2","月草的赐慧", GachaType.AvatarEventWish)
        {
            UpOrangeList = new() { "纳西妲" },
            UpPurpleList = new() { "雷泽", "诺艾尔", "班尼特" },
            Banner = "https://upload-bbs.mihoyo.com/upload/2022/10/28/75276539/e8fc2e301748ac67c906249eeba59c87_3811539008548485227.jpg",
            From = new(2022, 11, 2, 6, 0, 0, UTC8),
            To = new(2022, 11, 18, 17, 59, 59, UTC8)
        },
        new("3.2","焰色天河", GachaType.AvatarEventWish2)
        {
            UpOrangeList = new() { "宵宫" },
            UpPurpleList = new() { "雷泽", "诺艾尔", "班尼特" },
            Banner = "https://upload-bbs.mihoyo.com/upload/2022/10/28/75276539/bbbdbb4d9c405bda1bf01221a58d73b1_2881454750368093517.jpg",
            From = new(2022, 11, 2, 6, 0, 0, UTC8),
            To = new(2022, 11, 18, 17, 59, 59, UTC8)
        },
        new("3.2","神铸赋形", GachaType.WeaponEventWish)
        {
            UpOrangeList = new() { "千夜浮梦", "飞雷之弦振" },
            UpPurpleList = new() { "笛剑", "流浪乐章", "祭礼大剑", "弓藏", "西风长枪" },
            Banner = "https://upload-bbs.mihoyo.com/upload/2022/10/28/75276539/9c3b064db4f56168bb059d6e893c7fac_7903938881339590215.jpg",
            From = new(2022, 11, 2, 6, 0, 0, UTC8),
            To = new(2022, 11, 18, 17, 59, 59, UTC8)
        },

        //3.2下半
        new("3.2","华紫樱绯", GachaType.AvatarEventWish)
        {
            UpOrangeList = new() { "八重神子" },
            UpPurpleList = new() { "莱依拉", "托马", "鹿野院平藏" },
            Banner = "https://upload-bbs.mihoyo.com/upload/2022/11/14/75276539/bb82ed25f817318641a1dc1bb904dfaa_8197331366260922626.jpg",
            From = new(2022, 11, 18, 18, 0, 0, UTC8),
            To = new(2022, 12, 6, 14, 59, 59, UTC8)
        },
        new("3.2","暂别冬都", GachaType.AvatarEventWish2)
        {
            UpOrangeList = new() { "达达利亚" },
            UpPurpleList = new() { "莱依拉", "托马", "鹿野院平藏" },
            Banner = "https://upload-bbs.mihoyo.com/upload/2022/11/14/75276539/f315c063e2bb97bc7a62c9bb28bcbde7_6647071877413651364.jpg",
            From = new(2022, 11, 18, 18, 0, 0, UTC8),
            To = new(2022, 12, 6, 14, 59, 59, UTC8)
        },
        new("3.2","神铸赋形", GachaType.WeaponEventWish)
        {
            UpOrangeList = new() { "神乐之真意", "冬极白星" },
            UpPurpleList = new() { "西风剑", "西风秘典", "钟剑", "绝弦", "匣里灭辰" },
            Banner = "https://upload-bbs.mihoyo.com/upload/2022/11/14/75276539/367bdd944eb3f0e63cf7364710d05210_4988150181253091247.jpg",
            From = new(2022, 11, 18, 18, 0, 0, UTC8),
            To = new(2022, 12, 6, 14, 59, 59, UTC8)
        },

        //3.3上半
        new("3.3","余火变相", GachaType.AvatarEventWish)
        {
            UpOrangeList = new() { "流浪者" },
            UpPurpleList = new() { "珐露珊", "五郎", "烟绯" },
            Banner = "https://upload-bbs.miyoushe.com/upload/2022/12/03/75276539/af4b887e1b389cc340814f77ae698e3c_2177076150869613166.jpg",
            From = new(2022, 12, 7, 6, 0, 0, UTC8),
            To = new(2022, 12, 27, 17, 59, 59, UTC8)
        },
        new("3.3","鬼门斗宴", GachaType.AvatarEventWish2)
        {
            UpOrangeList = new() { "荒泷一斗" },
            UpPurpleList = new() { "珐露珊", "五郎", "烟绯" },
            Banner = "https://upload-bbs.miyoushe.com/upload/2022/12/03/75276539/455207c28113b5201d20dee6bc197e75_1790690252600215486.jpg",
            From = new(2022, 12, 7, 6, 0, 0, UTC8),
            To = new(2022, 12, 27, 17, 59, 59, UTC8)
        },
        new("3.3","神铸赋形", GachaType.WeaponEventWish)
        {
            UpOrangeList = new() { "图莱杜拉的回忆", "赤角石溃杵" },
            UpPurpleList = new() { "祭礼剑", "昭心", "西风大剑", "西风猎弓", "断浪长鳍" },
            Banner = "https://upload-bbs.miyoushe.com/upload/2022/12/03/75276539/2fba7b9b63635bf7bf08c4285ebe50ab_3073688490510066332.jpg",
            From = new(2022, 12, 7, 6, 0, 0, UTC8),
            To = new(2022, 12, 27, 17, 59, 59, UTC8)
        },

        //3.3下半
        new("3.3","影寂天下人", GachaType.AvatarEventWish)
        {
            UpOrangeList = new() { "雷电将军" },
            UpPurpleList = new() { "罗莎莉亚", "早柚", "九条裟罗" },
            Banner = "https://upload-bbs.miyoushe.com/upload/2022/12/21/75276539/fb67b909782e05ea827b119ff8f0c547_8117006675605513898.jpg",
        ...
        new("3.3","苍流踏花", GachaType.AvatarEventWish2)
        {
            UpOrangeList = new() { "神里绫人" },
            UpPurpleList = new() { "罗莎莉亚", "早柚", "九条裟罗" },
            Banner = "https://upload-bbs.miyoushe.com/upload/2022/12/21/75276539/0ab4ee8e14d4888805e1315d929b01c0_6841742765500457118.jpg",
            From = new(2022, 12, 27, 18, 0, 0, UTC8),
            To = new(2023, 1, 17, 14, 59, 59, UTC8)
        },
        new("3.3","神铸赋形", GachaType.WeaponEventWish)
        {
            UpOrangeList = new() { "波乱月白经津", "薙草之稻光" },
            UpPurpleList = new() { "匣里龙吟", "祭礼残章", "恶王丸", "曚云之月", "西风长枪" },
            Banner = "https://upload-bbs.miyoushe.com/upload/2022/12/21/75276539/7ff854d64488b96e5d84f3b08afe80c1_5803523662812557161.jpg",
            From = new(2022, 12, 27, 18, 0, 0, UTC8),
            To = new(2023, 1, 17, 14, 59, 59, UTC8)
        },

        //3.4上半
        new("3.4","敕诫枢谋", GachaType.AvatarEventWish)
        {
            UpOrangeList = new() { "艾尔海森" },
            UpPurpleList = new() { "瑶瑶", "云堇", "辛焱" },
            Banner = "https://upload-bbs.miyoushe.com/upload/2023/01/13/75276539/5648e99e80ec87d0b3977378a38e021f_4848053752090849514.jpg",
            From = new(2023, 1, 18, 6, 0, 0, UTC8),
            To = new(2023, 2, 7, 17, 59, 59, UTC8)
        },
        new("3.4","烟火之邀", GachaType.AvatarEventWish2)
        {
            UpOrangeList = new() { "魈" },
            UpPurpleList = new() { "瑶瑶", "云堇", "辛焱" },
            Banner = "https://upload-bbs.miyoushe.com/upload/2023/01/13/75276539/eb26f86e8eb62c528126f13f998658c2_233357724180408195.jpg",
            From = new(2023, 1, 18, 6, 0, 0, UTC8),
            To = new(2023, 2, 7, 17, 59, 59, UTC8)
        },
        new("3.4","神铸赋形", GachaType.WeaponEventWish)
        {
            UpOrangeList = new() { "裁叶萃光", "和璞鸢" },
            UpPurpleList = new() { "笛剑", "流浪乐章", "雨裁", "祭礼弓", "千岩长枪" },
            Banner = "https://upload-bbs.miyoushe.com/upload/2023/01/13/75276539/cf6b3bd2e382a9b90370f21ead517d43_2400596938038719618.jpg",
            From = new(2023, 1, 18, 6, 0, 0, UTC8),
            To = new(2023, 2, 7, 17, 59, 59, UTC8)
        },
